...
/** MRZ istisna / ICAO → KBS ülke kodu. */
...
/** ULKE: KBS ülke kodu (max 5; Türkiye = TC). */
...
/** Doğum: YYYY-MM-DD. Salt tarih dilimlenir; ISO datetime İstanbul günü. */
export function normalizeKbsBirthDate(value: string | null | undefined): string | null {
  if (!value) return null;
  const s = String(value).trim();
  if (!s) return null;

  // Tam gün (timezone kayması yok)
  ...
  // ISO datetime → İstanbul takvim günü (UTC slice yanlış gün üretir)
...
/** Oda no: "Oda 101", "101." gibi gürültüyü temizle. */
...
/** Pasaport/belge no: boşluksuz, büyük harf, max 20. Harfler korunur (AP902390). */
...
/** AP902390 → harf+rakam pasaport; T.C. / YKN değil. */
...
/**
 * Pasaportta ayrı seri yoksa: harf öneki (AP902390 → AP).
 * Böylece BELGESERI=BELGENO kopyası bazı Jandarma redlerinde düşer.
 */
