...
export default function PartnerMessagesScreen() {
  ...
  const [rows, setRows] = useState<ConversationWithMeta[]>([]);
  ...
  const removeConversation = (item: ConversationWithMeta) => {
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.75}
                onPress={() =>
                  router.push({ pathname: '/partner/chat/[id]', params: { id: item.id, name: item.name || 'Sohbet' } })
                }
                onLongPress={() => removeConversation(item)}
              >
