...
import { fetchKbsCaptureNotifyStaffIds, saveKbsCaptureNotifyStaffIds } from '@/lib/kbsCaptureSettings';

type StaffRow = { id: string; full_name: string | null; role: string };

export default function AdminKbsCaptureNotifyScreen() {
  const T = adminTheme;
    ...
    try {
      const [{ data: staffData, error: staffErr }, notifyIds] = await Promise.all([
        supabase
          ...
          .eq('is_active', true)
          .order('full_name'),
        fetchKbsCaptureNotifyStaffIds(orgScoped),
      ]);
      if (staffErr) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const res = await saveKbsCaptureNotifyStaffIds(orgScoped, [...selected]);
      if (!res.ok) {
        Alert.alert('Hata', res.message);
        return;
      }
      Alert.alert('Kaydedildi', 'Kimlik çekim bildirimi alıcıları güncellendi.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <View style={styles.root}>
      ...
      <Text style={styles.hint}>
        Kimlik/pasaport çekildiğinde bildirim yalnızca seçili personele gider. Hiç seçim yoksa bildirim gönderilmez.
      </Text>
      {loading ? (
