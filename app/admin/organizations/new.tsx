...
export default function AdminOrganizationNewScreen() {
  ...
  const save = async () => {
    ...
    Alert.alert('Kaydedildi', 'Yeni otel olusturuldu.', [{ text: 'Tamam', onPress: () => router.replace('/admin/organizations') }]);
  };

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Text style={styles.label}>Otel Adi</Text>
      <TextInput style={styles.input} value={name} onChangeText={(v) => { setName(v); if (!slug) setSlug(toSlug(v)); }} />
      ...
      <Text style={styles.label}>Varsayilan Para Birimi</Text>
      <TextInput style={styles.input} value={currencyCode} onChangeText={setCurrencyCode} autoCapitalize="characters" />
      <TouchableOpacity style={styles.saveBtn} onPress={save} disabled={saving}>
        {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Otel Olustur</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
