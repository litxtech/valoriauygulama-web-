...
export default function KitchenExpensesListScreen() {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('kitchen_expenses').select('id, entry_date, category, amount, description, supplier_name').order('entry_date', { ascending: false }).limit(50);
    setRows((data ?? []) as Row[]);
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.addFab} onPress={() => router.push('/staff/kitchen-ops/expenses/new' as never)}>
        <Ionicons name="add" size={22} color="#fff" /><Text style={styles.addFabText}>Gider Gir</Text>
      </TouchableOpacity>
      <FlatList
        data={rows}
        keyExtractor={(r) => r.id}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <View style={styles.row}><Text style={styles.amount}>{fmtKitchenMoney(Number(item.amount))}</Text><Text style={styles.date}>{formatDateShort(item.entry_date)}</Text></View>
...
const styles = StyleSheet.create({
  ...
  addFab: { flexDirection: 'row', alignItems: 'center', gap: 8, margin: 16, backgroundColor: '#ea580c', borderRadius: 14, paddingVertical: 14, justifyContent: 'center' },
