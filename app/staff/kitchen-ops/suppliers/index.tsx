...
export default function KitchenSuppliersListScreen() {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('kitchen_supplier_debts').select('id, supplier_name, amount, paid_amount, due_date, status').order('created_at', { ascending: false }).limit(50);
    setRows((data ?? []) as Row[]);
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.addFab} onPress={() => router.push('/staff/kitchen-ops/suppliers/new' as never)}>
        <Ionicons name="add" size={22} color="#fff" /><Text style={styles.addFabText}>Borç Kaydı</Text>
      </TouchableOpacity>
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[styles.card, overdue && styles.overdue]}>
              ...
              <Text style={styles.meta}>{STATUS_LABELS[item.status]} {item.due_date ? `· Vade: ${formatDateShort(item.due_date)}` : ''}</Text>
            </View>
          );
        }}
...
const styles = StyleSheet.create({
  ...
  addFab: { flexDirection: 'row', alignItems: 'center', gap: 8, margin: 16, backgroundColor: '#7c3aed', borderRadius: 14, paddingVertical: 14, justifyContent: 'center' },
