...
export default function KitchenPersonnelListScreen() {
  ...
  const fetchItems = useCallback(async () => {
    const { data } = await supabase
      ...
      .select('id, staff_name, staff_role, amount, payment_type, entry_date')
  ...
  const { items: rows, loading, refreshing, refresh } = useCachedList<Row>({
    cacheKey: 'kitchen-personnel-payments',
    fetchItems,
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.addFab} onPress={() => router.push('/staff/kitchen-ops/personnel/new' as never)}>
        ...
        <Text style={styles.addFabText}>Ödeme Kaydı</Text>
      </TouchableOpacity>
      <FlatList
        data={rows}
        keyExtractor={(r) => r.id}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>
              {TYPE_LABELS[item.payment_type]} · {formatDateShort(item.entry_date)}
            </Text>
          </View>
        )}
