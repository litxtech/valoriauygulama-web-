...
export default function KitchenSettlementsListScreen() {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('kitchen_settlements').select('id, payer_name, payee_name, amount, method, status, created_at').order('created_at', { ascending: false }).limit(50);
    setRows((data ?? []) as Row[]);
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.addFab} onPress={() => router.push('/staff/kitchen-ops/settlements/new' as never)}>
        <Ionicons name="add" size={22} color="#fff" /><Text style={styles.addFabText}>Ödeme / Mahsup</Text>
      </TouchableOpacity>
      <FlatList data={rows} keyExtractor={(r) => r.id} contentContainerStyle={styles.list}
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>{item.payer_name ?? '?'} → {item.payee_name ?? '?'} · {item.method}</Text>
            <Text style={styles.date}>{formatDateShort(item.created_at)} · {item.status}</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  addFab: { flexDirection: 'row', alignItems: 'center', gap: 8, margin: 16, backgroundColor: '#b45309', borderRadius: 14, paddingVertical: 14, justifyContent: 'center' },
