-- Partner kahvaltı kaydı: bildirimleri istemci yerine DB trigger ile gönder (523 / RLS önleme)

BEGIN;

CREATE OR REPLACE FUNCTION public.breakfast_partner_open_balance()
RETURNS numeric
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.breakfast_partner_open_balance() TO authenticated;
...
BEGIN
  ...
  IF v_kitchen_ids IS NOT NULL AND array_length(v_kitchen_ids, 1) > 0 THEN
    ...
    IF v_filtered IS NOT NULL AND array_length(v_filtered, 1) > 0 THEN
      ...
      SELECT sid, NULL, v_title, v_body, 'staff', 'breakfast_partner_entry', v_payload, NULL, 'both', now()
...
DROP TRIGGER IF EXISTS trg_breakfast_partner_entry_notify ON public.breakfast_partner_daily_entries;
CREATE TRIGGER trg_breakfast_partner_entry_notify
  AFTER INSERT OR UPDATE OF guest_count, line_total, note
  ON public.breakfast_partner_daily_entries
  FOR EACH ROW
  EXECUTE FUNCTION public.breakfast_partner_notify_entry_after_save();
