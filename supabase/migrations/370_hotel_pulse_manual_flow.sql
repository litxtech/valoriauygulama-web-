-- Bugünün Akışı: admin manuel oda listeleri
ALTER TABLE public.hotel_pulse_config
  ADD COLUMN IF NOT EXISTS flow_source text NOT NULL DEFAULT 'live' CHECK (flow_source IN ('live', 'manual')),
  ADD COLUMN IF NOT EXISTS manual_flow_check_in_rooms text,
  ADD COLUMN IF NOT EXISTS manual_flow_check_out_rooms text,
  ADD COLUMN IF NOT EXISTS manual_flow_upcoming_rooms text,
  ADD COLUMN IF NOT EXISTS manual_flow_late_checkout_rooms text;

CREATE OR REPLACE FUNCTION public.hotel_pulse_csv_to_flow_json(p_csv text, p_status text DEFAULT 'checked_in')
RETURNS jsonb
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', 'mf-' || replace(trim(s.tok), ' ', ''),
        'room_number', trim(s.tok),
        'check_in_at', null,
        'check_out_at', null,
        'status', p_status
      )
  ...
  FROM (
    SELECT trim(tok) AS tok, row_number() OVER () AS ord
    FROM regexp_split_to_table(COALESCE(p_csv, ''), E'[,;\\n]+') AS tok
    WHERE trim(tok) <> ''
  ) s;
$$;

COMMENT ON FUNCTION public.hotel_pulse_csv_to_flow_json(text, text) IS
  'Virgül/noktalı virgül/satır ile ayrılmış oda numaralarını misafir akış JSON dizisine çevirir.';

-- get_guest_hotel_pulse: akış listeleri için manuel kaynak desteği
