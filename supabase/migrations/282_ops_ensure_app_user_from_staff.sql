-- Admin/manager staff → ops.app_users (KBS Edge + gateway auth için).
...
BEGIN
  ...
  SELECT s.full_name, s.role, s.is_active, o.slug, o.name
  INTO v_staff
  FROM public.staff s
  ...
  IF v_staff.role NOT IN ('admin', 'manager') THEN
...
REVOKE ALL ON FUNCTION ops.ensure_app_user_for_auth(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION ops.ensure_app_user_for_auth(uuid) TO service_role;

COMMENT ON FUNCTION ops.ensure_app_user_for_auth(uuid) IS
  'JWT kullanıcısı için ops.app_users oluşturur (public.staff admin/manager). Edge KBS kayıt fonksiyonu çağırır.';
