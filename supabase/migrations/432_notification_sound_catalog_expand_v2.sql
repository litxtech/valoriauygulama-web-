-- Bildirim ses kataloğu: bölüm kuralları, bahşiş, ödeme, teknik varlık, misafir ev hizmeti
...
BEGIN
  ...
  VALUES
    ...
    (p_organization_id, 'department_rule', 'Bölüm kuralları', 'Kural yayını ve hatırlatmalar', 'default', 'default', 'valoria_ns_department_rule', v_staff),
    (p_organization_id, 'staff_tip', 'Bahşiş', 'Bahşiş ve teşekkür bildirimleri', 'default', 'default', 'valoria_ns_staff_tip', v_staff),
    (p_organization_id, 'payment', 'Ödeme', 'Genel ödeme alındı bildirimleri', 'default', 'default', 'valoria_ns_payment', v_staff),
    (p_organization_id, 'technical_asset', 'Teknik varlık / arıza', 'Arıza, bakım ve tesis durumu', 'default', 'default', 'valoria_ns_technical_asset', v_staff),
    (p_organization_id, 'guest_service_request', 'Misafir ev hizmeti', 'Oda talebi, menü siparişi', 'default', 'default', 'valoria_ns_guest_service_request', v_staff)
  ON CONFLICT (organization_id, feature_key) DO NOTHING;
END;
$$;

COMMENT ON FUNCTION public.ensure_notification_sound_settings(uuid) IS
  'Organizasyon için tüm bildirim sesi feature_key kayıtlarını oluşturur (31 kategori).';
