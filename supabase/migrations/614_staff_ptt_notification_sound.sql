-- Bas-konuş: ilk oturum bildiriminde walkie sesi + özel Android kanalı
  ...
  SELECT coalesce(
    (
      SELECT jsonb_build_object(
        ...
        'priority', CASE
          WHEN s.feature_key IN ('emergency_alert', 'staff_ptt') THEN 'high'
          ELSE 'normal'
        END
      )
    ...
    jsonb_build_object(
      ...
      'priority', CASE
        WHEN p_feature_key IN ('emergency_alert', 'staff_ptt') THEN 'high'
        ELSE 'normal'
      END
    )
  );
...
SELECT
  ...
  'staff_ptt',
  'Bas-konuş (telsiz)',
  'Personel telsizinde biri konuşunca gelen bildirim',
  'walkie_ptt_open.wav',
  'walkie_ptt_open.wav',
  'valoria_ns_staff_ptt_v2',
  true,
  2
...
SET
  ...
  END,
  android_channel_version = CASE
    WHEN coalesce(nullif(trim(notification_sound_settings.sound_file_url), ''), '') = ''
      THEN EXCLUDED.android_channel_version
    ELSE notification_sound_settings.android_channel_version
  END,
  is_active = true,
  updated_at = NOW();
