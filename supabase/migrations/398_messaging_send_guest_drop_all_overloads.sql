-- PGRST203: messaging_send_message_guest için kalan tüm imzaları kaldır, tek imza bırak, PostgREST önbelleğini yenile.
...
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'messaging_send_message_guest'
  LOOP
    EXECUTE 'DROP FUNCTION ' || r.sig::text || ' CASCADE';
  END LOOP;
END $drop$;

CREATE FUNCTION public.messaging_send_message_guest(
  p_app_token TEXT,
...
COMMENT ON FUNCTION public.messaging_send_message_guest(TEXT, UUID, TEXT, VARCHAR, TEXT, TEXT, JSONB) IS
  'Misafir mesaj gönderir (tek imza; PGRST203 yok).';
