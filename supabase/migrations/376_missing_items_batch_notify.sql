-- Eksik bildirimleri: rapor kalemleri tek tek değil, liste halinde tek bildirim.
-- Hatırlatmalar rapor bazlı; rapor kalemi giderildiğinde tek tek bildirim yok.
...
DECLARE
  ...
  v_area_label text;
BEGIN
  -- Rapor kalemleri: açılış bildirimi create_missing_item_report sonunda tek sefer gider.
  IF TG_OP = 'INSERT' AND NEW.report_id IS NOT NULL THEN
    RETURN NEW;
  END IF;

  -- Rapor kalemi tek tek giderildi: bildirim rapor tamamlanınca gider.
...
BEGIN
  ...
  v_area_label := CASE NEW.area WHEN 'kitchen' THEN 'Mutfak' ELSE 'Otel' END;
  v_title := 'Eksik Var (' || v_area_label || ') tamamlandi: ' || NEW.item_count::text || ' kalem';
...
DECLARE
  ...
  v_sent integer := 0;
BEGIN
  -- Açık raporlar: tek hatırlatma, kalemler liste halinde
  FOR v_report IN
    SELECT
      r.id,
      r.organization_id,
  ...
  LOOP
    ...
    v_title := 'Eksik Var hatirlatma (' || v_area_label || '): ' || v_report.item_count::text || ' kalem';
  ...
  -- Eski tekli kayıtlar (report_id yok)
  FOR v_row IN
  ...
  LOOP
    ...
    v_area_label := CASE v_row.area WHEN 'kitchen' THEN 'Mutfak' ELSE 'Otel' END;
    v_title := 'Eksik Var hatirlatma (' || v_area_label || '): ' || v_row.title;
