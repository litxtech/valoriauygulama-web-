-- Muhasebe PDF / yazdırma üst başlığı (işletme bazında; boşsa işletme adı)

BEGIN;

ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS finance_report_brand text;

COMMENT ON COLUMN public.organizations.finance_report_brand IS
  'Gelir/gider fişi ve cari rapor PDF üst başlığı. Boşsa organizations.name kullanılır.';
...
CREATE OR REPLACE FUNCTION public.update_organization_admin(
  ...
  p_finance_report_brand text DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    finance_report_brand = NULLIF(trim(COALESCE(p_finance_report_brand, '')), '')
  WHERE id = p_org_id;
...
GRANT EXECUTE ON FUNCTION public.update_organization_admin(
  uuid, text, text, text, text, text, text, text, text, boolean, uuid, text, text
) TO authenticated;
