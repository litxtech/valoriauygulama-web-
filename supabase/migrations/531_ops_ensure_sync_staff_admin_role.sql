-- Mevcut ops.app_users kaydı receptionist kalmışsa Valoria staff.admin ile sync et.
-- Admin panel KBS yetkileri: ikinci kez ops.role=admin zorunluluğu olmasın diye rol güncellenir.
...
COMMENT ON FUNCTION ops.ensure_app_user_for_auth(uuid) IS
  'JWT kullanıcısı için ops.app_users oluşturur; staff.admin ise mevcut kaydı admin rolüne yükseltir.';
