-- Teşekkür bildirimi: misafir bildirimlerinde yalnızca guest_id (staff_id NULL olmalı)
...
BEGIN
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'send_staff_tip_thank_you notification skipped: %', SQLERRM;
  END;
END;
$$;

COMMENT ON FUNCTION public.send_staff_tip_thank_you IS
  'Bahşiş alan personel misafire tek seferlik teşekkür gönderir; misafire in-app bildirim (guest_id only).';
