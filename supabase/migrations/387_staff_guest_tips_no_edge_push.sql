-- Bahşiş: Edge push RPC içinden çıkar (522); misafir doğrudan INSERT + sadece DB bildirimi
...
BEGIN
  ...
  INSERT INTO public.notifications (
    ...
    p_staff_id, p_guest_id, v_title_staff, v_body_staff, 'staff', 'staff_tip', v_payload, 'in_app', now()
  );
...
DROP TRIGGER IF EXISTS trg_staff_tips_after_insert_notify ON public.staff_tips;
CREATE TRIGGER trg_staff_tips_after_insert_notify
  AFTER INSERT ON public.staff_tips
  FOR EACH ROW
  EXECUTE FUNCTION public.staff_tips_after_insert_notify();
...
CREATE POLICY staff_tips_guest_insert ON public.staff_tips
  ...
  WITH CHECK (
    ...
    AND amount >= 10
    AND amount <= 50000
    AND payment_method IN ('room_charge', 'card_at_desk', 'cash_at_desk')
    AND EXISTS (
...
GRANT INSERT ON public.staff_tips TO authenticated;

COMMENT ON FUNCTION public.notify_staff_tip_created IS
  'Bahşiş sonrası in-app bildirim satırları (Edge push yok — 522 önlenir).';
