-- Yaş / doluluk göstergesi: sade metin (abartısız)
...
BEGIN
  ...
  IF v_count IS NULL OR v_count < 1 OR v_avg IS NULL THEN
    RETURN QUERY SELECT
      ...
      (upper(left(v_occ, 1)) || substr(v_occ, 2) || '. İlk rezervasyonlardan biri olabilirsiniz.')::text;
    RETURN;
  END IF;
  ...
  v_hint := upper(left(v_occ, 1)) || substr(v_occ, 2) || '.';
